...
function Swipe(container, options) {
  ...
  function setup() {
    var newWidth = options.width || container.getBoundingClientRect().width || container.offsetWidth;
    ...
    while(pos--) {
      ...
      slide.setAttribute('data-index', String(pos));
  ...
  function slide(to, slideSpeed) {
    ...
    var displayIndex = (index + 1 > actualAmountOfSlides ? index - actualAmountOfSlides : index);
    visibleThree();
  ...
  var events = {
    ...
    start: function(event) {
      if (browser.pointer && (event.pointerType !== pointerType)) {
        return;
      }
  ...
  if (browser.addEventListener) {
    ...
    if (browser.touch || browser.pointer) {
      element.addEventListener(eventNames.START, events, false);
    }
